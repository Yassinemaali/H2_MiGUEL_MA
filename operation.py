...
class Operator:
    ...
    def export_core_data(self):
        ...
        print(f"✅ Export der Kerndaten abgeschlossen: {core_file}")
    ...
    def plot_daily_system_behavior(self, day=None):
        ...
        plt.plot(df_day.index, df_day['Load [W]'], label='Last [W]')
        plt.plot(df_day.index, df_day['PV_Production [W]'], label='PV-Produktion [W]')
        if 'ES_1 [W]' in df_day.columns:
            plt.plot(df_day.index, df_day['ES_1 [W]'], label='Batterie [W]')
        if 'Electrolyser_1 [W]' in df_day.columns:
            plt.plot(df_day.index, df_day['Electrolyser_1 [W]'], label='Elektrolyseur [W]')
        if 'H2_Storage level [kg]' in df_day.columns:
            plt.plot(df_day.index, df_day['H2_Storage level [kg]'] * 1000, label='H2-Speicher [g]')
        if 'FuelCell_2 [W]' in df_day.columns:
            plt.plot(df_day.index, df_day['FuelCell_2 [W]'], label='FuelCell [W]')

        plt.title(f'Systemverhalten am {start_date.date()}')
        plt.xlabel('Zeit')
        plt.ylabel('Leistung / Menge')
