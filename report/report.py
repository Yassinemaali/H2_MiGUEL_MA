...
class Report:
    ...
    def create_pdf(self):
        ...
        self.pdf_file.set_keywords('EnerSHelF, Renewable Energy, Energy systems, MiGUEL, Amazonas , '
                                   'PV-H2-Hybrid systems')
    ...
    def introduction_summary(self):
        ...
        # Retrieve Battery Storage values
        battery_charge = sum([value for key, value in energy_parameters.items() if 'Battery_charge' in key])
        battery_discharge = abs(sum([value for key, value in energy_parameters.items() if 'Battery_discharge' in key]))

        # Retrieve Hydrogen Storage values
        h2_electrolyzer_input = sum(
            [value for key, value in energy_parameters.items() if 'Electrolyzer' in key])  # Charge
        h2_fuelcell_output = abs(
            sum([value for key, value in energy_parameters.items() if 'FuelCell' in key]))  # Discharge

        # Retrieve H2 Storage buffer (if relevant)
        h2_storage_charge = sum([value for key, value in energy_parameters.items() if 'H2Storage_charge' in key])
        h2_storage_discharge = abs(
            sum([value for key, value in energy_parameters.items() if 'H2Storage_discharge' in key]))
        ...
        summary = system_status + \
                  ...
                  f"{abs(es_charge):,} kWh; the Hydrogen System provide { abs (h2_fuelcell_output)} kWh and are charged with {abs(h2_electrolyzer_input)} kWh; "\
                  f"  - H2 Storage charged with: {h2_storage_charge:,} kWh and discharged with: {h2_storage_discharge:,} kWh\n" \\
                  f" The table below shows the energy systems key parameters. The parameters will " \ 
        ...
        '''
        summary = system_status + \
                  f" The PV system(s) account for {pv_percentage}% ({pv_energy:,} kWh); " \
                  f"The wind turbine(s) account for {wt_percentage}% ({wt_energy:,} kWh); " \
                  f"The grid accounts for {grid_percentage}% ({grid_energy:,} kWh); " \
                  f"The hydrogen system(s) includes:\n" \
                  f"   - Electrolyzer energy input: {h2_electrolyzer_input:,} kWh\n" \
                  f"   - Fuel Cell energy output: {h2_fuelcell_output:,} kWh\n" \
                  f"   - H2 Storage charged with: {h2_storage_charge:,} kWh and discharged with: {h2_storage_discharge:,} kWh\n" \
                  f" The battery system provided {battery_discharge:,} kWh and was charged with {battery_charge:,} kWh.\n" \
                  f" The table below shows the energy system's key parameters, which will be described in detail in the upcoming report.\n\n"
    ...
    def energy_consumption(self):
        ...
        if self.env.grid_connection:
            ...
            co2_emission = ['CO2 emissions [t]', 'grid connection'
                            '-' , '-']
    ...
    def energy_supply(self):
        ...
        h2storage_header = ['Component', 'Name', 'Capacity', 'storage level']
        h2storage_values = [h2storage_header]


        for h2_comp in self.env.h2_components:
            if isinstance(h2_comp, H2Storage):
                h2storage_values.append(['H2 Storage', h2_comp.name, h2_comp.capacity, h2_comp.get_storage_level()])
        h2storage_components = [[''], h2storage_values]
        self.pdf_file.create_table(file=self.pdf_file,
                                   table=h2storage_components,
                                   padding=2)
        self.pdf_file.create_table(file=self.pdf_file,
    ...
    def dispatch(self):
        ...
        for h2comp in env.h2_components:
            print(f"DEBBUG h2 COMPONENTS: {h2comp}")
            if isinstance(h2comp, Electrolyser):
                columns.append(h2comp.name + '[W]')
            elif isinstance(h2comp, H2Storage):
                columns.append(h2comp.name + '[W]')
            elif isinstance(h2comp, FuelCell):
                columns.append(h2comp.name + '[W]')
        self.create_plot(df=self.operator.df,
                         columns=columns,
                         file_name='dispatch',
    ...
    def create_input_parameter(self):
        ...
        data = {'Parameter': ['Project name', 'City', 'ZIP Code', 'State', 'Country', 'Country Code', 'Latitude [°]',
                ...
                'Value': [env.name, env.address[0], env.address[1], env.address[2], env.address[3], env.address[4],
                          ...
                          env.electricity_price, env.feed_in, env.pv_feed_in_tariff,
                          env.wt_feed_in_tariff, env.lifetime, env.d_rate, env.co2_grid]}
    ...
    def create_plot(self, df: pd.DataFrame, columns: list, file_name: str, x_label: str = None, y_label: str = None,
        ...
        """
        print("Verfügbare Spalten im DataFrame:", df.columns.tolist())
    ...
    def create_sankey(self):
        ...
        label = ['PV', 'PV self consumption', 'PV charge',
                 ...
                 'Battery storage discharge', 'Electrolyser', 'H2 Storage', 'Fuel Cell', 'Load', 'Feed-in', 'Losses']
        node = dict(pad=15,
                    thickness=20,
        ...
        h2_production = 0  # Elektrolyseur Output
        h2_storage_in = 0  # H2 Speicherung
        h2_storage_out = 0  # Entnahme aus H2 Speicher
        fuelcell_output = 0  # Stromproduktion aus H2

        for h2_comp in env.h2_components:
            if isinstance(h2_comp, Electrolyser):
                h2_production += op.df[f'{h2_comp.name}: H2 Production [kg]'].sum() * 33.33  # kg → kWh
            elif isinstance(h2_comp, H2Storage):
                h2_storage_in += op.df[f'{h2_comp.name}: H2 Inflow [kg]'].sum() * 33.33
                h2_storage_out += op.df[f'{h2_comp.name}: H2 Outflow [kg]'].sum() * 33.33
            elif isinstance(h2_comp, FuelCell):
                fuelcell_output += op.df[f'{h2_comp.name}: Power Output [kW]'].sum() * time_factor
        ...
        value = [grid_production,
                 ...
                 h2_production, h2_storage_in, fuelcell_output]
